    ...
    <div ref="gc" class="goods-content">
        <ul ref="gcUl">
            <li :ref="'gcLi'+index" v-for="(item, index) in goods" :key="index">
                <h3>{{item.name}}</h3>
                <div class="goods-content-info" v-for="(te, index2) in item.foods" :key="index2">
                    ...
                    <div class="btn">
                        <div class="btn-wrap">
                            <template v-if="$store.getters.buyListCount[te.name]">
                                <span @click="decPro(te.name, te.price)" class="dec"></span>
                                <span class="number">{{$store.getters.buyListCount[te.name]}}</span>
                            </template>
                            <span @click="addPro(te.name, te.price)" class="inc"></span>
                        </div>
                    </div>
                </div>
            </li>
